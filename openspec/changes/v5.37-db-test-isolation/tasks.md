...
- [x] `_isolate_db` autouse fixture in `tests/conftest.py` pointing
      `db._DB_PATH` at a temp path.
- [x] `_guard_real_cache_db` session-scoped fixture: count rows in the real
      cache before and after the run, fail if they changed.
- [x] Read-only connection; any `sqlite3.Error` disables the check rather
      than failing the run.

## Verified

- [x] Real `cache.db`: 474 rows before a full suite run, **474 after**
      (previously grew every run).
