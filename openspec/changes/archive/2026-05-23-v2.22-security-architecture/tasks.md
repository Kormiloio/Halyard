...
- [x] Import `xml.sax.saxutils.escape` in `service.py`
- [x] Wrap `project_dir` interpolation with `escape(str(project_dir))`
- [x] Audit plist template for any other interpolated values; wrap each
- [x] `tests/test_plist_xml_injection.py`
  - [x] `test_plist_xml_safe_project_dir` — normal path produces valid XML
  - [x] `test_plist_xml_injection_lt_gt` — project_dir with < > produces valid XML
  - [x] `test_plist_xml_injection_ampersand` — project_dir with & produces valid XML
  - [x] `test_plist_xml_injection_combined` — all three characters, parsed by xml.etree

## D-3: org.toml change detection

- [x] Add `_org_hash_path(hub_dir: Path, org_id: str | None = None)` helper
  returning a hub/org-scoped path under `~/.halyard/org-hashes/`
- [x] Implement `_check_org_hash(content: bytes) -> None`
...
- [x] Call `_check_org_hash()` in the org.toml load path
- [x] Review note: do not use a single global `~/.halyard/org-hash.txt`;
  switching between hubs/orgs must not warn or overwrite another baseline
- [x] `tests/test_org_hash.py`
  - [x] `test_first_run_no_warning` — no hash file → no warning, hash written
  - [x] `test_unchanged_org_no_warning` — same content → no warning
  - [x] `test_changed_org_warning_emitted` — changed content → warning printed
  - [x] `test_changed_org_hash_updated` — new hash stored after change
  - [x] `test_two_hubs_have_independent_org_hashes` — loading hub A then hub B
    does not warn or overwrite hub A's baseline

## D-5: Pricing table hash pinning

- [x] Add `_pricing_hash_path()` helper returning `~/.halyard/pricing-hash.txt`
- [x] Wrap pricing table HTTP fetch to verify Content-Length / completeness
- [x] Implement `_check_pricing_hash(body: bytes) -> bool`
...
- [x] On first fetch with no stored hash: accept validated table and persist
  table + hash
- [x] On hash mismatch with existing stored hash: print warning and abort
  persistence unless caller explicitly accepts the changed table
- [x] Add CLI acceptance path for changed table, e.g.
  `halyard update-pricing --accept-changed-table`; non-interactive runs fail
  closed unless the flag is present
- [x] On truncated response: abort; do not update local table or hash
- [x] `tests/test_pricing_hash.py`
  - [x] `test_first_fetch_no_warning` — no stored hash → accept silently
  - [x] `test_matching_hash_no_warning` — same body → no warning
  - [x] `test_changed_pricing_table_warning` — changed body → warning printed
  - [x] `test_changed_pricing_table_does_not_overwrite_without_accept`
  - [x] `test_changed_pricing_table_accept_flag_overwrites_and_updates_hash`
  - [x] `test_truncated_response_no_overwrite` — incomplete response → local table unchanged
  - [x] `test_truncated_response_hash_not_updated` — incomplete response → hash file unchanged
...
- [x] `tests/test_session_roundtrip.py`
  - [x] `test_session_roundtrip_all_fields` — write → parse → assert all fields identical
  - [x] `test_session_roundtrip_optional_fields_none` — None fields omitted and parse back as None
